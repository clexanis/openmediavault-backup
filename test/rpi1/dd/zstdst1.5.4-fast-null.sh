...
if [ "${part_type}" = "gpt" ]; then
    ...
    if [ -e "${esppart}" ]; then
      ...
      dd if=${esppart} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd --single-thread --fast -c > "${backupFile}"
    else
...
case ${method} in
    dd)
        ...
        dd if=${devicefile} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd --single-thread --fast -c > "${backupFile}"
        status=( "${PIPESTATUS[@]}" )
        echo "dd exit code = ${status[0]}"
        echo "/usr/local/src/zstd/build/cmake/programs/zstd --single-thread exit code = ${status[1]}"
        ...
        if [ -n "${bootpart}" ]; then
            ...
            dd if=${bootpart} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd --single-thread --fast -c > "${backupFile}"
    ...
    ddfull)
        ...
        dd if=${root} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd --single-thread --fast -c > "${backupFile}"
        status=( "${PIPESTATUS[@]}" )
        echo "dd exit code = ${status[0]}"
        echo "/usr/local/src/zstd/build/cmake/programs/zstd --single-thread exit code = ${status[1]}"
        if [[ ${status[0]} -gt 0 ]] || [[ ${status[1]} -gt 0 ]]; then
