...
if [ "${part_type}" = "gpt" ]; then
    ...
    if [ -e "${esppart}" ]; then
      ...
      dd if=${esppart} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast -c > "${backupDir}/${OMV_BACKUP_FILE_PREFIX}-${date}.espdd.gz"
    else
...
case ${method} in
    dd)
        ...
        dd if=${devicefile} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast -c > "${backupDir}/${OMV_BACKUP_FILE_PREFIX}-${date}.dd.gz"
        status=( "${PIPESTATUS[@]}" )
        _log "dd exit code = ${status[0]}"
        _log "/usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast exit code = ${status[1]}"
        ...
        if [ -n "${bootpart}" ]; then
            ...
            dd if=${bootpart} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast -c > "${backupDir}/${OMV_BACKUP_FILE_PREFIX}-${date}_boot.dd.gz"
    ...
    ddfull)
        ...
        dd if=${root} bs=1M conv=sync,noerror status=progress | /usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast -c > "${backupDir}/${OMV_BACKUP_FILE_PREFIX}-${date}.ddfull.gz"
        status=( "${PIPESTATUS[@]}" )
        _log "dd exit code = ${status[0]}"
        _log "/usr/local/src/zstd/build/cmake/programs/zstd -T0 --fast exit code = ${status[1]}"
        if [[ ${status[0]} -gt 0 ]] || [[ ${status[1]} -gt 0 ]]; then
